...
| `github_project_analysis_status` | analysis diagnostics | GitHub analysis callback through `AnalysisState` helper | report generation, diagnostics | no | source-empty status | distinguishes empty source, filtered candidates, and analysis failure | no dedicated artifact | yes | derived |
