...
if TYPE_CHECKING:
    ...
    class _LegacyStateModule(Protocol):
        CompanyAnalysis: type[Any]
        MacroImpactAnalysis: type[Any]
        MarketSignalAnalysis: type[Any]
        NormalizedEvent: type[Any]
        PaperAnalysis: type[Any]
        Prediction: type[Any]
        PredictionUpdate: type[Any]
        ProjectAnalysis: type[Any]
        RawEvent: type[Any]
        Report: type[Any]
        SocialSignalAnalysis: type[Any]
        TechDailyState: type[Any]
        TopicSummary: type[Any]

    def _load_legacy_state_module() -> _LegacyStateModule:
        existing = sys.modules.get("state")
        if existing is not None:
            return cast(_LegacyStateModule, existing)

        state_path = Path(__file__).resolve().parents[3] / "scripts" / "state.py"
        spec = importlib.util.spec_from_file_location("state", state_path)
        if spec is None or spec.loader is None:
            raise ModuleNotFoundError(f"Could not load legacy state module from {state_path}")

        module = importlib.util.module_from_spec(spec)
        sys.modules["state"] = module
        spec.loader.exec_module(module)
        return cast(_LegacyStateModule, module)

    try:
        import state as _legacy_state_module
    except ModuleNotFoundError as exc:
        if exc.name != "state":
            raise
        _legacy_state = _load_legacy_state_module()
    else:
        _legacy_state = cast(_LegacyStateModule, _legacy_state_module)

    CompanyAnalysis = _legacy_state.CompanyAnalysis
    MacroImpactAnalysis = _legacy_state.MacroImpactAnalysis
    MarketSignalAnalysis = _legacy_state.MarketSignalAnalysis
    NormalizedEvent = _legacy_state.NormalizedEvent
    PaperAnalysis = _legacy_state.PaperAnalysis
    Prediction = _legacy_state.Prediction
    PredictionUpdate = _legacy_state.PredictionUpdate
    ProjectAnalysis = _legacy_state.ProjectAnalysis
    RawEvent = _legacy_state.RawEvent
    Report = _legacy_state.Report
    SocialSignalAnalysis = _legacy_state.SocialSignalAnalysis
    TechDailyState = _legacy_state.TechDailyState
    TopicSummary = _legacy_state.TopicSummary
...
def apply_new_predictions_result(
    ...
    *,
    ...
    if signal_level is not None:
        prediction_state.signal_level = signal_level
    prediction_state.apply_to_tech_daily_state(state)
    return prediction_state
